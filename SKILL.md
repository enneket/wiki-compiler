...
1. **同步对比**：
   运行环境钩子脚本 `python3 <INSTALL_DIR>/scripts/sync_manifest.py --raw "$RAW_DIR" --wiki "$WIKI_DIR"`。
...
4. **收尾 (更新账本)**：
   全部写入完成后，运行 `python3 <INSTALL_DIR>/scripts/sync_manifest.py --raw "$RAW_DIR" --wiki "$WIKI_DIR" --mark-done` 刷新 `ledger` 账本，确保这些文件已被彻底封存、未来不再重新编译。
...
1. 如果上下文中已存在或你能查到 `$RAW_DIR` 和 `$WIKI_DIR`，在沉思前，请顺手跑一次扫描：`python3 <INSTALL_DIR>/scripts/sync_manifest.py --raw "$RAW_DIR" --wiki "$WIKI_DIR"`。
...
3. 运行脚本 `python3 <INSTALL_DIR>/scripts/wiki_dreamer.py --wiki "$WIKI_DIR" --limit 3`。
...
7. 运行 `python3 <INSTALL_DIR>/scripts/health_vis_engine.py --wiki "$WIKI_DIR" --auto-stub`。
...
1. **Map (全库检索与精读)**：
   运行 `python3 <INSTALL_DIR>/scripts/wiki_weaver.py --wiki "$WIKI_DIR" --query "搜索词"`。
...
```
python3 <INSTALL_DIR>/scripts/query_wiki.py --wiki "$WIKI_DIR" --query "搜索关键词" --top 5
```
