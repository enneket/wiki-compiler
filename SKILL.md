---
name: wiki-compiler
description: 知识库自动编译器。支持通过命令 `/wiki-compiler` 或“收录到知识库”触发。同时包含 `/wiki-dream`（做梦机制/沉思），以在空闲时融合现有知识节点。利用幂等引擎防止重复生成，并融合 Dataview/Marp 原生可视化方案。
---
...
2. 如果扫描显示有**待处理的增量名单**（漏网遗忘的数据），你必须在这个梦境的开场，**主动向用户发出提醒并请求授权**：“提示：发现有一些你丢进去但遗忘处理的原始资料（包含 XXX），是否需要我借助现在的做梦时间顺手帮你把它们编译入库？”若用户同意，请先走一遍工作流 A。
